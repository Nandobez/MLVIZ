...
export function getDomainFromEmailWithoutAnySubdomain(email) {
...
export function onHandleAppearanceModeChange(className) {
...
export function onHandleAppearanceChange(className) {
  const body = document.body;
...
export function onHandleFontChange(className) {
  const body = document.body;
...
export const isFocusableElement = (element) => {
...
export const findNextFocusable = (element, direction = 'next') => {
...
export const findFocusableDescendant = (container, currentFocused = null, direction = 'next') => {
  if (!container) return null;

  const focusableElements = Array.from(container.querySelectorAll('a[href], button, input, select, textarea, [tabindex]:not([tabindex="-1"]), [contenteditable="true"]'));
